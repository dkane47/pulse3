...
const Intro2 = () => {
  ...
  React.useEffect(() => {
    let numbers = [2,3,4,5,10]; //list of fact families for practice. 6 represents 6s, 7s, and 8s, and 0 and 1 are mixed in during practice mode
    ...
    setLogic((prevLogic) => ({ //use random list to generate a random sequence of problems
      ...prevLogic,
      sequence: [[s[0],0], [s[1],0], [s[2],0], [s[3],0], [s[4],0], //multiplication
      [s[0],1], [s[1],1], [s[2],1], [s[3],1], [s[4],1],  //division
      [s[0],2], [s[1],2], [s[2],2], [s[3],2], [s[4],2],  //fill-in-the-blank multiplication
      [s[0],3], [s[1],3], [s[2],3], [s[3],3], [s[4],3], //division
      [9,0], [9,1], [6,2], [6,3],  //random mix of operations
      [6,0], [6,1], [9,2], [9,3],  
      [11,0], [9,1], [11,2], [6,3],  
      [6,0], [11,1], [9,2], [11,3],  
  ...
  React.useEffect(() => { //function for level up and timeToTarget adjustments
    if (logic.step % 5 === 0 && logic.step > 1 && logic.step < 22 && !displaySettings.switched) { //level up logic
  ...
  const targetedPractice = () => { //function to switch from analyzing mode to practice mode
    ...
    for (let i = 0; i < 47; i++) {
      if (i % 2 === 0) {
        ...
        const easyProblems = [...solvedProblems,[2,0],[2,0],[10,0],[10,0],[1,0],[0,0]] //append 2s, 10s, 1s, 0s
        const randomIndex = Math.floor(Math.random() * (logic.step + 6)); 
